...
import { inviteModeEnabled, PORTAL_ADMIN_COOKIE, checkAdminPassphrase, makeAdminCookieValue, isValidAdminCookie, sessionCookieOptions } from "./auth";
...
export async function loginAdmin(passphrase: string): Promise<boolean> {
  if (!checkAdminPassphrase(passphrase)) return false;
  (await cookies()).set(PORTAL_ADMIN_COOKIE, makeAdminCookieValue(), sessionCookieOptions());
  return true;
}

export async function logoutAdmin(): Promise<void> {
  (await cookies()).delete(PORTAL_ADMIN_COOKIE);
}

export async function isAdminAuthed(): Promise<boolean> {
  return isValidAdminCookie((await cookies()).get(PORTAL_ADMIN_COOKIE)?.value);
}
